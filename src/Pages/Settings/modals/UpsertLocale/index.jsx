    ...
    if (!spinner) {
      ...
      setLocales()
        .onOk(result => {
          ...
          showNotify(
            "success",
            t("Locales updated successfully")
          );
          closeModal();
        })
        .onServerError(result => {
          ...
          showNotify(
            "error",
            t("SETTINGS_ADD_LOCALE_ON_SERVER_ERROR")
          );
        })
        .onBadRequest(result => {
          toggleSpinner(false);
          showNotify(
            "error",
            t("SETTINGS_ADD_LOCALE_ON_BAD_REQUEST")
          );
        })
        .unAuthorized(result => {
          toggleSpinner(false);
          showNotify(
            "error",
            t("SETTINGS_ADD_LOCALE_UN_AUTHORIZED")
          );
        })
        .notFound(result => {
          toggleSpinner(false);
          showNotify(
            "error",
            t("SETTINGS_ADD_LOCALE_NOT_FOUND")
          );
        })
