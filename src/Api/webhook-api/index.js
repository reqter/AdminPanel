...
const getAllWebhookUrl = baseUrl + config.REACT_APP_APPS_GET_ALL
...
export function getWebhooks () {
  ...
  const _call = async spaceId => {
    try {
      const url = getAllWebhookUrl + '?id=' + spaceId
...
export function addWebhook () {
  ...
  const _call = async (spaceId, webhook) => {
    try {
      debugger
...
export function deleteWebhook () {
  ...
  const _call = async (spaceId, webhookId) => {
    try {
      const url = removeUrl
      const token = storageManager.getItem('token')
      var rawResponse = await fetch(url, {
